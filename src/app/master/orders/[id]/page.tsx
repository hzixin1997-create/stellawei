...
export default function MasterOrderDetail({ params }: { params: { id: string } }) {
  ...
  if (!order) {
    return (
      <div className="min-h-screen bg-gradient-to-br from-[#1a1a2e] via-[#16213e] to-[#0f3460] flex items-center justify-center">
        <div className="text-white/50">订单不存在</div>
      </div>
    );
  }
  ...
  const statusMap: Record<string, { label: string; color: string; icon: any }> = {
    ...
    assigned: { label: "待回复", color: "bg-orange-500/20 text-orange-300", icon: AlertCircle },
    "in_progress": { label: "处理中", color: "bg-purple-500/20 text-purple-300", icon: Clock },
    completed: { label: "已完成", color: "bg-green-100 text-green-300", icon: CheckCircle },
    ...
    <div className="min-h-screen bg-gradient-to-br from-[#1a1a2e] via-[#16213e] to-[#0f3460]">
      ...
      <header className="bg-black/40 backdrop-blur-md border-b border-white/10 px-6 py-4">
        <div className="max-w-3xl mx-auto flex items-center gap-4">
          <Link href="/master/orders" className="text-white/50 hover:text-white/70">
            <ArrowLeft size={20} />
          </Link>
          <div>
            <h1 className="text-lg font-medium text-white">订单详情</h1>
          </div>
        </div>
      </header>
      ...
      <main className="max-w-3xl mx-auto px-6 py-8">
        {/* 订单信息 */}
        <div className="bg-black/40 backdrop-blur-sm border border-white/10 p-6 mb-6">
          <div className="flex items-start justify-between mb-4">
            <div>
              <h2 className="text-xl font-semibold text-white">{order.service_name}</h2>
        ...
        {order.user_question && (
          <div className="bg-black/40 backdrop-blur-sm border border-white/10 p-6 mb-6">
            <h3 className="text-sm font-medium text-white/50 uppercase tracking-wide mb-3">客户问题</h3>
            <div className="bg-white/5 rounded-lg p-4">
              <p className="text-white/70 whitespace-pre-wrap">{order.user_question}</p>
            </div>
        ...
        {(order.status === "assigned" || order.status === "in_progress" || order.status === "completed") && (
          <div className="bg-black/40 backdrop-blur-sm border border-white/10 p-6 mb-6">
            ...
            <textarea
              ...
              className="w-full h-48 p-4 border border-white/10 rounded-lg resize-none focus:outline-none focus:ring-2 focus:ring-violet-500/20 focus:border-violet-500 text-white/70"
            ...
            {order.status !== "completed" && (
              <div className="flex gap-3 mt-4">
                <button
                  ...
                  className="px-6 py-2.5 bg-violet-600 text-white rounded-lg font-medium hover:bg-violet-700 disabled:opacity-50 disabled:cursor-not-allowed transition-colors"
                >
                  {saving ? "保存中..." : "提交回复"}
                </button>
                <button
        ...
        {order.status === "completed" && (
          <div className="bg-green-500/10 border border-green-500/30 rounded-xl p-6 text-center">
            ...
            <p className="text-green-300 font-medium">此订单已完成</p>
          </div>
        )}
      </main>
    </div>
