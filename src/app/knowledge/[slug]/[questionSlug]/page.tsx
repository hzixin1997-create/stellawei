...
export default function KnowledgeArticlePage({ params }: Props) {
  ...
  if (questionSlug === "when-will-i-meet-my-soulmate") {
    return <SoulmateArticlePage slug={slug} />;
  }

  return <DynamicArticlePage slug={slug} questionSlug={questionSlug} />;
}

// ==================== SOULMATE ARTICLE (Original Format Preserved) ====================

function SoulmateArticlePage({ slug }: { slug: string }) {
  ...
  const c = {
    ...
    cta: {
      textLine1: isZh ? "每段关系都是独特的。" : "Every relationship is unique.",
      textLine2: isZh
      ...
      <div className="min-h-screen bg-[#0a0a1a]">
        ...
        <main className="pt-24 max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
          ...
          <p className="text-white/60 mb-8">
            {isZh ? "这篇知识文章正在撰写中，敬请期待。" : "This knowledge article is being written. Stay tuned."}
          </p>
          <Link href={`/knowledge/${topic?.slug}`} className="inline-flex items-center px-6 py-3 bg-stellawei-purple text-white font-medium rounded-xl hover:bg-stellawei-purple/90 transition-colors">
